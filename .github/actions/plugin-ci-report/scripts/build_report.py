#!/usr/bin/env python3
"""Build a unified per-plugin CI report payload."""
...
def main() -> int:
    plugin = os.getenv("PLUGIN_NAME", "").strip()
    project_path = os.getenv("PROJECT_PATH", "").strip() or plugin
    ...
    output_file = os.getenv("OUTPUT_FILE", "").strip() or f"ci-plugin-result-{plugin}.json"
    ...
    project_id = f"{repo_owner}_{plugin}" if repo_owner and plugin else ""
    project_locator = quote(f"custom+48578/{project_id}", safe="") if project_id else ""
    encoded_branch = quote(branch, safe="")
    encoded_revision = quote(revision, safe="")
    fossa_report_url = (
        f"https://app.fossa.com/projects/{project_locator}/refs/branch/{encoded_branch}/{encoded_revision}"
        if project_id and branch and revision
        else ""
    )
    ...
    payload: dict[str, Any] = {
        ...
        # Backward-compatible fields for current aggregators.
