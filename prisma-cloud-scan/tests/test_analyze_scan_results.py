...
class TestAnalyzeScanResults(unittest.TestCase):
    ...
    def test_main_defers_to_guardian_without_upload(self):
        # arm64 case: no Guardian upload on this lane, but the caller defers the
        # vulnerability verdict to Guardian. Local publish-date policy must not
        # block, and the lane is reported as Guardian-managed.
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            results = {
                "results": [
